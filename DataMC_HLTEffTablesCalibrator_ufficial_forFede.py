...
def CalibrationTables_HLT(df, inputType, channel,  year, leptons):
    ...
    if(leptons == 'mm'):
        ...
        eff_HLT_L0M, df_HLT_L0Meff = TagAndProbe_HLT_onlynumerator(dfL0M, year, modelHLT, '{}-HLT-{}'.format(inputType,year+modelHLT+"_L0M"), Adaflag, None, VERB)
        #If we want L0 reweighted
        '''
        #eff_HLT_L0M, df_HLT_L0Meff = TagAndProbe_HLT_onlynumerator(dfL0M, year, modelHLT, '-L0Mrw-{}'.format(year+modelHLT+"_L0M"), False, "wL0M_maxPT", VERB) 
        #eff_HLT_L0Mw, df_HLT_L0Meffw = TagAndProbe_HLT(dfL0M, year, modelHLT, '-BDT-{}'.format(year+modelHLT+"L0M"), False, 'weights_gb2', VERB) 
        '''

        eff_HLT_L0H, df_HLT_L0Heff = TagAndProbe_HLT_onlynumerator(dfL0Hin, year, modelHLT, '{}-HLT-{}'.format(inputType,year+modelHLT+"_L0H"), Adaflag, None, VERB) 
        '''
        #eff_HLT_L0H, df_HLT_L0Heff = TagAndProbe_HLT_onlynumerator(dfL0H, year, modelHLT, '-L0Hrw-{}'.format(year+modelHLT+"_L0H"), False, 'wL0H_alsoL0M', VERB) 
        #eff_HLT_L0Hw, df_HLT_L0Heffw = TagAndProbe_HLT(dfL0H, year, modelHLT, '-BDT-{}'.format(year+modelHLT+"L0H"), False, 'weights_gb2', VERB) 
        '''

        eff_HLT_L0TIS, df_HLT_L0TISeff = TagAndProbe_HLT_onlynumerator(dfL0TISin, year, modelHLT, '{}-HLT-{}'.format(inputType,year+modelHLT+"_L0TIS"), Adaflag, None, VERB) 
        '''
        #eff_HLT_L0TIS, df_HLT_L0TISeff = TagAndProbe_HLT_onlynumerator(dfL0TIS, year, modelHLT, '-L0TISrw-{}'.format(year+modelHLT+"_L0TIS"), False, "wL0TIS_alsoL0MH", VERB) 
        ...
        eff_HLT_L0E, df_HLT_L0Eeff = TagAndProbe_HLT_onlynumerator(dfL0E, year, modelHLT, '{}-HLT-{}'.format(inputType,year+modelHLT+"_L0E"), Adaflag, None, VERB)
        #If we want L0 reweighted
        '''
        #eff_HLT_L0E, df_HLT_L0Eeff = TagAndProbe_HLT_onlynumerator(dfL0E, year, modelHLT, '-L0Erw-{}'.format(year+modelHLT+"_L0E"), False, "wL0E_maxPT", VERB) 
        #eff_HLT_L0Ew, df_HLT_L0Eeffw = TagAndProbe_HLT(dfL0E, year, modelHLT, '-BDT-{}'.format(year+modelHLT+"L0E"), False, 'weights_gb2', VERB) 
        '''

        eff_HLT_L0H, df_HLT_L0Heff = TagAndProbe_HLT_onlynumerator(dfL0Hin, year, modelHLT, '{}-HLT-{}'.format(inputType,year+modelHLT+"_L0H"), Adaflag, None, VERB) 
        '''
        #eff_HLT_L0H, df_HLT_L0Heff = TagAndProbe_HLT_onlynumerator(dfL0H, year, modelHLT, '-L0Hrw-{}'.format(year+modelHLT+"_L0H"), False, 'wL0H_alsoL0E', VERB) 
        #eff_HLT_L0Hw, df_HLT_L0Heffw = TagAndProbe_HLT(dfL0H, year, modelHLT, '-BDT-{}'.format(year+modelHLT+"L0H"), False, 'weights_gb2', VERB) 
        '''

        eff_HLT_L0TIS, df_HLT_L0TISeff = TagAndProbe_HLT_onlynumerator(dfL0TISin, year, modelHLT, '{}-HLT-{}'.format(inputType,year+modelHLT+"_L0TIS"), Adaflag, None, VERB) 
        '''
        #eff_HLT_L0TIS, df_HLT_L0TISeff = TagAndProbe_HLT_onlynumerator(dfL0TIS, year, modelHLT, '-L0TISrw-{}'.format(year+modelHLT+"_L0TIS"), False, "wL0TIS_alsoL0EH", VERB) 
        #eff_HLT_L0TISw, df_HLT_L0TISeffw = TagAndProbe_HLT(dfDataL0TIS, year, modelHLT, '-BDT-{}'.format(year+modelHLT+"L0TIS"), False, 'weights_gb2', VERB) 
        '''
...
def Open_files_for_TriggerCalibration():

    ######################################
    directory = '/disk/data12/lhcb/flionett/LHCb/Analysis/Ntuples/MC/{}/DFs/WithHOP/ParallelSplitOutput/Meerkat/'.format(channel)


    joblist = []
    for imag in ['Down','Up']:
        joblist.extend(GetJob( channel, year, imag, inputType))
    ...
    for ijob in joblist:
        ...
        for ifile in listfiles(directory):
            ...
            if ((".h5" in ifile) & (".h5." not in ifile) & (ifile.split("-")[-2] == ijob)):
                ...
                try:
                    store = pd.HDFStore(dir_baseMC+ifile)
                    ...
                    if(store.keys()):
                        ...
                        try:
                            ...
                            if(leptons == 'ee'):
                                data = CalibSelection_E(data, TM, version, low_val)
                            if(leptons == 'mm'):
                                data = CalibSelection_M(data, TM)
