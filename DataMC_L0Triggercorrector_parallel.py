...
if __name__ == "__main__" :
    ...
    Description:
    Open the single h5 file and execute the Singlefile_L0TriggerCorrector.py script. This can be done in parallel in two ways: using the "multiprocessing" python module (tested and working) or submitting batch jobs on TORQUE (no tested yet).

    How to run it:
    Choose the year of the desired MC/data correction of the trigger and execute!
    > python DataMC_L0Triggercorrector_parallel.py -options [y,l,v,low,,c] --options[test, TM, VERB, b] | tee Log_DataMC_L0Triggercorrector_parallel_[options]_[options].log
    ...
    if (user == 'M'):
        ...
        directory = '/home/hep/matzeni/gangadir/Analysis/DFs/MC/B02Kst0Jpsi2{}/DFs/WithHOP/ParallelSplitOutput/L0HLT/'.format(leptons)
        ...
        for ijob in joblist:
            ...
            for ifile in listfiles(directory):
                ...
                if ((".h5" in ifile) & (".h5." not in ifile) & (ifile.split("-")[-2] == ijob)):
                    ...
                    if(batch):
                        os.system('sbatch -p express --nodes=1  -t 02:00:00  --ntasks-per-node=2 --export=ifile={},year={},Tag_name={}   submit_L0Correction.slurm'.format(ifile, year,Tag_name))
                        ...
                        #Singlefile_L0TriggerCorrector(directory+ifile, jobsDict[ijob]["year"], Tag_name)
